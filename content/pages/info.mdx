---
title: Pro zájemce
blocks:
  - image:
      src: /uploads/259246_4109486109662_338693137_o.jpg
      alt: zavist
    quote: |
      Začínáme ve školním roce 2023/24! \
      \
      Vítáni jsou i předškoláci!
    author: ''
    color: default
    _template: testimonial
  - body: "V září 2023 otvíráme třídu pro 15 dětí ve věku od 5 do 7 let. Tedy pro věkové rozpětí předškoláků až 2. třída.\_\n\nPokud máte doma předškoláka a nejste si jisti výběrem školy, máte u nás příležitost vyzkoušet si, zda vašemu dítěti bude vyhovovat náš způsob práce.\n"
    color: default
    _template: content
  - body: "# Kdy bude probíhat výuka?\_\n\nVýuka bude probíhat **4 dny v týdnu**, **od pondělí do čtvrtka**. Jeden den v týdnu bude vždy dnem výletním.\_\n\n### ​​Denní rozvrh\n\n* **8:30 - 9:00** příchod dětí\_\n* **9:00 - 12:00** dopolední blok\_\n* **12:00-12:30** oběd\_\n* **12:30-13:30** odpolední blok\_\n* **13:45-16:30** odpolední aktivity / družina - pokud bude zájem ze strany rodičů\n"
    color: default
    _template: content
  - body: "# Školné\_\n\nMěsíční příspěvek je 8 500 Kč.\n\nMaximální počet dětí je 15, a je pro nás důležité, aby dopoledne byli s dětmi vždy dva průvodci.\n"
    color: default
    _template: content
  - body: "# Prostory\_\n\nZázemí budeme mít v prostorách současné oční ordinace, která se stěhuje – Elišky Přemyslovny 449. Prostory jsou to hezké, dobře dostupné, ale bez zahrady. Hledáme tedy na Zbraslavi ještě prostor, kde by bylo možné vybudovat si komunitní školní zahradu.\n"
    color: default
    _template: content
  - body: "# Zápis\_\n\nPokud máte zájem přihlásit Vaše dítě do Habitatu, nebo být jen nezávazně informováni o aktuálním dění, neváhejte nám napsat na [zapis@habitatzbraslav.cz](mailto\\:zapis@habitatzbraslav.cz).\_\n\nDne 13. 4. proběhne na Zbraslavi setkání pro zájemce (místo upřesníme), kde bude možné potkat se osobně a zodpovědět veškeré otázky.\_\n"
    color: default
    _template: content
  - body: "### Jak plní děti zákonnou povinnost školní docházky?\_\n\nDěti jsou v režimu individuálního vzdělávání. Za individuální vzdělávání zodpovídá zákonný zástupce dítěte. Děti jsou zapsány a evidovány v ZŠ, která má právo poskytovat vzdělání na základě zařazení do sítě škol ČR vydaného MŠMT (tzv. Kmenová škola). Ve školním roce 2023/24 ještě nebudeme zajišťovat kmenovou ZŠ. V následujících letech už bychom ale takovou školu rádi\_nabídli a zajistili tím i povinné přezkoušení.\_\n"
    color: default
    _template: content
  - body: >
      # Kontakt


      Mgr. Alžběta Křížová


      +420 607 535 669


      [alzbeta.krizova@habitatzbraslav.cz](mailto\:alzbeta.krizova@habitatzbraslav.cz)
    color: default
    _template: content
---

















